            ...
            if slot == None and text != "\f":
                for dash in len(text):
                    if text[dash] == "-":
                        slot = text[:dash]
                        break
            course_name_raw = re.findall(r"[A-z0-9]{3,6}[0-9]{1,4}[A-z]{0,1}", text)
...
def fetch_text_timetable(text):
    data, slots = [], []
    slots += re.findall(
        r"[A-Z]{1,3}[0-9]{1,2}[\D]{1}[A-Z]{3,4}[0-9]{3,4}[A-Z]{0,1}[\D]{1}[A-Z]{2,3}[\D]{1}[A-Z]{2,4}[0-9]{2,4}[A-Z]{0,1}[\D]{1}[A-Z]{3}",
        text,
    )
